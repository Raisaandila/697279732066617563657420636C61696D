  ...
  for (const { walletAddress, captchaToken } of wallets) {
    try {
      ...
      if (data.success === true) {
        console.log(`✅ ${walletAddress.slice(0, 6)} | ${etc.timelog()} | Faucet successfully claimed.`);
      } else {
        console.warn(`⚠️ ${walletAddress.slice(0, 6)} | ${etc.timelog()} | Faucet response: ${JSON.stringify(data)}`);
      }
    } catch (err) {
      if (axios.isAxiosError(err) && err.response?.status) {
        console.error(`❌ ${walletAddress.slice(0, 6)} | ${etc.timelog()} | Rate limit : ${err.response.data.message}`);
      } else {
        console.error(`❌ ${walletAddress.slice(0, 6)} | ${etc.timelog()} | Error: ${err.message}  : ${JSON.stringify(err.response.data)}`);
      }
    }
